...
class CurveWithHeadingAndSpeed(CurveBase):
    ...
    def _expert_output(
        self, last_pos, last_heading, last_speed, expert_heading, expert_speed
    ):
        return self._normalize_state(
            [last_pos.x, last_pos.y, last_heading, last_speed]
    ...
    def reset(self):
        super()._reset()
        return self._normalize_state(
            [self.agent_position.x, self.agent_position.y, self.heading, self.speed]
        )
